class FoodMenusController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @food_menu.save
        ...
        format.json { render action: 'show', status: :created, location: @food_menu }
      else
  ...
  def destroy
    @food_menu.destroy
    respond_to do |format|
      format.html { redirect_to food_menus_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def food_menu_params
      params.require(:food_menu).permit(:name, :photo, :description, :price)
    end
end
